...
import InsuranceListActiveSellNav from './list-active-sell-nav';
import { Box, Button, Center } from '@chakra-ui/react';
import { Spinners } from '../../../spinners';
import { useRouter } from 'next/navigation';
import { BsDatabaseX } from 'react-icons/bs';
import { useListActiveSellInsuranceListByPrincipal } from '@/lib/hooks/canisters/insurance/list-active-insurances-by-principal';
import { formatDateTimeInput } from '../../trade/issue/issue-contract-form';
import { convertMilliSecondsToDateTime } from '@/lib/apis/utils';

export function ListActiveSellContracts() {
  const [isLoading, setLoading] = useState<boolean>(false);

  const router = useRouter();

  const [listInsurancesKey, loadingListInsuranceApi, list] =
    useListActiveSellInsuranceListByPrincipal();
              ...
              (list.length == 0 && (
                <>
                  <Box className="flex flex-row items-center justify-center gap-2 mt-40 lg:mt-60  text-bold font-sans">
                    <BsDatabaseX />
                    <Center>No Sell contract Found</Center>
                  </Box>
                </>
              ))}
          </div>
