...
export type ActivityNavProps = {
  setActivityState: (state: OptionsContractState) => void;
  setRefresh: () => void;
};
function ActivityNav({ setActivityState, setRefresh }: ActivityNavProps) {
  const [action, setAction] = useState(false);

  const setOptionsAssetsNavHandle = (nav: String) => {
    if (nav === 'OPEN') {
      setActivityState({ OPEN: null });
    } else if (nav === 'CLOSED') {
      setActivityState({ CLOSED: null });
    } else if (nav === 'EXECUTED') {
      setActivityState({ EXECUTED: null });
    } else if (nav === 'EXPIRED') {
      setActivityState({ EXPIRED: null });
    } else if (nav === 'OFFER') {
      setActivityState({ OFFER: null });
    }
  };
  ...
  const tabItems = [
...

    {
      icon: (
        <SiBitcoinsv
          className={
            ''
            // optionsAsset == OptionsAssets.CKBTC
            //   ? 'w-5 h-5 bg-indigo-400'
            //   : 'w-5 h-5'
          }
        />
      ),
    ...
    },
    {
      icon: (
        <FaEthereum
        //   className={
        //     optionsAsset == OptionsAssets.CKETH
        //       ? 'w-5 h-5 bg-indigo-400'
        //       : 'w-5 h-5'
        //   }
        />
